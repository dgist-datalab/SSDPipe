...
def main(args):
    ...
    for idx, i in enumerate(testset.targets):
        if i > args.num_class:#977/987/10000
            pass
        else:
            a.append(idx)
    ...
    if args.evaluate:
        ...
        for i, batch in enumerate(testloader):
            ...
            target_list = np.append(target_list, targets, axis = 0)
            output_list = np.append(output_list, output, axis = 0)
        print(len(target_list), len(output_list))
        np.save(args.target_name, target_list)
        np.save(args.output_name, output_list)
        exit()
    ...
    nod = int(len(trainset)*(args.percent/100))
    a = []
    b = [1]*nod + [0]*(len(trainset)-nod)
    random.shuffle(b)
    print(b[:100])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--percent', default=100, type=int,
                        help='the percentage of dataset used for training')
    parser.add_argument('--target_name', default='target', type=str, help='mini-batch size')
    parser.add_argument('--output_name', default='output', type=str, help='mini-batch size')
