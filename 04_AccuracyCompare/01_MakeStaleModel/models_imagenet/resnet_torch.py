...
from .load_dict_torch import load_state_dict_from_url
...
class ResNet(nn.Module):
    ...
    def __init__(self, args, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        # 2D-conv auto-encoder
        # if self.args.auto_encoder:
        self.ae_encoder = nn.Conv2d(512, int(512 / args.ae_compress_rate), 3, padding=1)
        self.pool = nn.MaxPool2d(2, 2)
        self.ae_decoder = nn.ConvTranspose2d(int(512 / args.ae_compress_rate), 512, 2, stride=2)
    ...
    def _forward_impl(self, x):
        ...
        if self.args.auto_encoder:
            x = self.relu(self.ae_encoder(x))
            # print("Encode:", x.shape)

            x = self.pool(x)
            # print(x.shape)

            x = self.relu(self.ae_decoder(x))
